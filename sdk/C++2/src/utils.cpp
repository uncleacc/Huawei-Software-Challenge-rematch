...
void robot_close_berth(int berth_id) {
    for (int j = 0; j < robot_num; ++j) {
        robot[j]->close(berth_id);
    }
}
...
/*
 * 在ts时刻(x, y, dir)方向的船是否可以放置，因为频繁被调用，所以重载了这个函数，直接传参vector
 */
/*bool can_place_boat(int x, int y, int idx, int ts) {
    if(ts > 15500) return false;
 ...
 * 在ts时刻(x, y, dir)方向的船是否可以放置，因为频繁被调用，所以重载了这个函数，直接传参vector
 */
bool can_place_boat(const vector<pair<int, int>> &points, int ts) {
    if(ts > 15500) return false;
    for(int i = 0; i < points.size(); i++) {
        if(!check_boat_can_loc(points[i].first, points[i].second, ts)) {
            return false;
 ...
 * 检测(x, y)处是否有slow点
 */
bool check_boat_loc_slow(const vector<pair<int, int>> &points) {
    bool res = false;
    for(int i = 0; i < points.size(); i++) {
        if(slow_points.find({points[i].first, points[i].second}) != slow_points.end()) res = true;
    }
...
void boat_set_obstacle(int x, int y, int dir, int ts) {
    ...
    for(int i = 0; i < points.size(); i++) {
        ...
        // info << "把" << points[i].first << " " << points[i].second << "在" << ts << "时刻设置为障碍" << endl;
    }
}
...
//重载版本
bool check_boat_can_loc_dir(const vector<pair<int, int>> &points) {
    for(int i = 0; i < points.size(); i++) {
        if(!is_sea(points[i].first, points[i].second)) return false;
    }
    return true;
...
void boat_close_berth(int berth_id) {
    for(int i = 0; i < boat_num; i++) {
...
bool locate_berth_area(int x, int y, int berth_id) {
    if (KaoBerth2ID.find({x, y}) != KaoBerth2ID.end()) {
        if(KaoBerth2ID.find({x, y})->second == berth_id)
            return true;
    }
    if(Berth2ID.find({x, y}) != Berth2ID.end() ) {
        if(Berth2ID.find({x, y})->second == berth_id)
            return true;
    }
    return false;
...
void berth_flood_algorithm(int id) {
    ...
    while (!q.empty()) {
        ...
        if (check_boat_loc_slow(points)) ndis ++;
        if (check_boat_can_loc_dir(points) && ndis < boat_to_berth_hCost[id][nx][ny][ndir]) {
...
void sea_flood_algorithm(int id, int sx, int sy) {
    ...
    while (!q.empty()) {
        ...
        if (check_boat_loc_slow(points)) ndis ++;
        if (check_boat_can_loc_dir(points) && ndis < boat_to_deliver_hCost[id][nx][ny][ndir]) {
...
void robot_berth_flood_algorithm(int berth_id) {
    ...
    robot_init_finished[berth_id] = true;
}

double get_metrics(int value, int distance, int dis_1, int dis_2, int vanishtime) {
    double r1[3] = {8,8,10}, r2[3] = {4,4,4}, r3[3] = {2,2,1};
    return r1[map_index] * value * 1.0 / 200 + r2[map_index] * (300 - distance) * 1.0 / 300 + r3[map_index] * (1000 - (vanishtime - step)) * 1.0 / 1000;

    // return value * 1.0 / (distance + 1);
}

void close_berth_at_time(int ts, int berth_id) {
    if(step >= ts) {
        boat_close_berth(berth_id);
        robot_close_berth(berth_id);
        closed_berth.push_back(berth_id);
    }
}